...
from simple_agent.services.rag_service import ingest_pdf_to_vector_store
...
async def upload_pdf(file: UploadFile, company: dict) -> dict:
    ...
    try:
        ...
        await db.database["documents"].update_one(
            ...
            {"$set": {"indexing_status": "indexed", "chunks_indexed": chunks_indexed, "indexed_at": datetime.now(timezone.utc)}},
        )
    except Exception as exc:
        await db.database["documents"].update_one(
            {"_id": result.inserted_id},
            {"$set": {"indexing_status": "failed", "indexing_error": str(exc)}},
        )
